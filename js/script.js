...
function addGreet() {
    ...
    let greet;
    if (hrs < 12)
        greet = 'Good Morning , Sun Shine !';
    else if (hrs >= 12 && hrs <= 17)
        greet = 'Good Afternoon, Super Star !';
    else if (hrs >= 17 && hrs <= 24)
        greet = 'Good Evening, Rock Star !';
    document.getElementById('greeting').innerText = greet;
    document.title = greet.split(',')[0] + yourName
}

function addQuotes() {
    getQuotes().then(data => {
        document.getElementById('headline').innerText = ` " ${data.content} "`
...
function getQuotes(endpoint, customConfig) {
    const config = {
        method: 'GET',
        "async": true,
        "crossDomain": true,
...
function addWeather() {
    ...
    getWeather().then(response => {
        const {current: {weather_descriptions}, request: {query}} = response;
        let appearanceProperty = "";
        switch (weather_descriptions[0]) {
            case "hurricane": //hurricane svg
            case "rain":
            case "cyclone":
                $("#canvas").css('display', 'block')
                appearanceProperty = getHurricane(weather_descriptions[0]);
                break;
            case "overcast-clouds": //overCast svg
                appearanceProperty = getOverCastClouds(weather_descriptions[0]);
                break;
            case "Partly cloudy": //overCast svg
                appearanceProperty = getOverCastClouds(weather_descriptions[0]);
                break;
            case "fog" : //fog-svg
                appearanceProperty = getFogWeather(weather_descriptions[0]);
                break;
            case "clear sky": //sunny svg
                appearanceProperty = getClearSky(weather_descriptions[0]);
                break;
            case "snow": //snow svg
                $("#weatherOverLay").append(snowElem())
                appearanceProperty = getSnow(weather_descriptions[0]);
                break;
            default:  //sunny svg
                appearanceProperty = getCleanWeather(weather_descriptions[0]);
                break;
        }
        $("#location").append(addLocation(query))
        $("#addWeather").append(appearanceProperty)
    })
}
...
function getLocation() {
    ...
    return window
        .fetch(`https://api.ipgeolocation.io/ipgeo?apiKey=${yourApiKey}&lang=en`, config).then(response => response.json());
}
...
function getWeather() {
    ...
    return getLocation().then((response) => {
        const {city} = response;
        return window
            .fetch(`http://api.weatherstack.com/current?access_key=${yourApiKey}&query=${city}`, config)
            .then(response => response.json())
    })
}

function getHurricane(weather_descriptions) {
    return `<a class="nav-link" rel="tooltip" title="${weather_descriptions}" data-placement="bottom">
                        <div id="hurricane-svg" class="svg-contain">
...
function getOverCastClouds(weather_descriptions) {
    return `
<a class="nav-link" rel="tooltip" title="${weather_descriptions}" data-placement="bottom"> 
    <div id="overcast-clouds" class="svg-contain">
...
function getCleanWeather(weather_descriptions) {
    return `
<a class="nav-link" rel="tooltip" title="${weather_descriptions}" data-placement="bottom">
    <div id="clear-sky-svg" class="svg-contain">
...
function getFogWeather(weather_descriptions) {
    return `
<a class="nav-link" rel="tooltip" title="${weather_descriptions}" data-placement="bottom">
    <div id="fog-svg" class="svg-contain">
...
function getClearSky(weather) {
    return `<a class="nav-link" rel="tooltip" title="${weather}" data-placement="bottom">
    <div id="clear-sky-svg" class="svg-contain">
        <svg version="1.1" class="clear-sky-svg" xmlns="http://www.w3.org/2000/svg"
             xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 72.3 52.6"
             style="enable-background:new 0 0 72.3 52.6;" xml:space="preserve">
...
function getSnow(weather) {
    return `
    <a class="nav-link" rel="tooltip" title="${weather}" data-placement="bottom">
    <div id="snow-svg" class="svg-contain">
...
function addLocation(location) {
    return `
     <a class="nav-link" rel="tooltip" title="${location}" data-placement="bottom"
